...
async function startSatDevs() {
    const SatDevs = NexusNwIncConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['SELFBOT - Satganz Devs','Safari','1.0.0'],
        auth: state
    })
    ...
    SatDevs.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./SatganzDevs")(SatDevs, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    // Get Profile Picture Group
       ...
       ppgc = 'https://shortlink.SatDevsarridho.my.id/rg1oT'
       }
...
//welcome\\
        let nama = await SatDevs.getName(num)
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/8bbe8a7de5c351dfcb077.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    SatDevs.sendMessage(anu.id, { image: Kon, contextInfo: { mentionedJid: [num] }, caption: `
⭐✑ Hi👋 @${num.split("@")[0]},
...
⭐✑ Welcome To Our Comfortable Happy😋, Sometimes Loud😜, Usually Messy🤥, Full Of Love🥰, HOME😌!!`} )
                    ...
                    SatDevs.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `⭐✑ @${num.split("@")[0]} Left ${metadata.subject}
...
⭐✑ I'm Not Sure If It Was A Goodbye Charm, But It Was Fun While It Lasted 😌✨` })
    ...
    SatDevs.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); SatDevs.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("🐦Connection Replaced, Another New Session Opened, Please Close Current Session First"); SatDevs.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`🐦Device Logged Out, Please Scan Again And Run.`); SatDevs.logout(); }
            ...
            else SatDevs.end(`🐦Unknown DisconnectReason: ${reason}|${connection}`)
        }
     ...
     * @returns
     */
    SatDevs.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: SatDevs.waUploadToServer })
